...
import { getBooksByBookshelf } from "@/lib/gutendex-client"
import {
    type Book,
    type BrowseSort,
...
interface BookshelfBookListProps {
    shelfName: string
    initialPage: number
    sort: BrowseSort
...
export function BookshelfBookList({
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            const key = `bookshelf-${shelfName}|${nextPage}|${sort}`
            const data =
                getCached(key) ??
                (await getBooksByBookshelf(shelfName, nextPage, sort))
    ...
    }, [hasNext, loading, nextPage, sort, shelfName])
            ...
            {books.length > 0 ? (
                ...
                <div className="flex flex-col items-center gap-4 py-16 text-center">
                    ...
                    <p className="text-lg text-muted-foreground">
                        No books found in this reading list.
                    </p>
                </div>
            )}
        </>
