...
import { getBookById, getCoverUrl, formatAuthorName, formatDownloadCount } from "@/lib/gutendex"
import {
  ArrowLeft01Icon,
  BookOpen01Icon,
  ArrowDown01Icon,
...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      ...
      description: book.summaries[0] || `Read ${book.title} for free on Project Gutenberg`,
    }
  } catch {
...
export default async function BookDetailPage({
  ...
  const authorName = mainAuthor ? formatAuthorName(mainAuthor) : "Unknown Author"
      ...
      <div className="grid gap-8 md:grid-cols-[280px_1fr] lg:grid-cols-[320px_1fr]">
        ...
        <div className="flex flex-col gap-6">
          <div>
            ...
            {mainAuthor && (mainAuthor.birth_year || mainAuthor.death_year) && (
              <div className="mt-1 flex items-center gap-1 text-sm text-muted-foreground">
                <Calendar01Icon className="size-3.5" />
                {mainAuthor.birth_year || "?"} &ndash; {mainAuthor.death_year || "?"}
              </div>
            )}
          </div>
          ...
          {book.subjects.length > 0 && (
            <div>
              <h3 className="font-heading text-lg font-semibold">Subjects</h3>
              <Separator className="my-2" />
          ...
          {book.bookshelves.length > 0 && (
            <div>
              <h3 className="font-heading text-lg font-semibold">Bookshelves</h3>
              <Separator className="my-2" />
              <div className="flex flex-wrap gap-1.5">
                {book.bookshelves.map((shelf) => (
                  <Badge key={shelf} variant="secondary" className="text-xs">
                    {shelf}
                  </Badge>
                ))}
              </div>
          ...
          {book.translators.length > 0 && (
            <div>
              <h3 className="font-heading text-lg font-semibold">Translators</h3>
              <Separator className="my-2" />
              <div className="flex flex-wrap gap-1.5">
